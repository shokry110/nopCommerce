--upgrade scripts from sarayetel 2.60 to sarayetel 2.65
...
--Full-text support
IF EXISTS (SELECT 1 FROM [Setting] WHERE [name] = N'commonsettings.usefulltextsearch' AND [value]=N'true')
BEGIN
	DECLARE @FullTextIsSupported int
	SET @FullTextIsSupported = CASE SERVERPROPERTY('IsFullTextInstalled')
	WHEN 1 THEN 
	...
	ELSE 0
	END
	IF (@FullTextIsSupported = 1)
	BEGIN
		--create full-text indexes for product tags
		DECLARE @create_index_text nvarchar(4000)
		SET @create_index_text = '
		IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductTag]''))
			CREATE FULLTEXT INDEX ON [ProductTag]([Name])
